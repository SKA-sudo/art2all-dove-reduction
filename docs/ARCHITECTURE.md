...
The semantic perception engine is organized as a multi-stage pipeline. Each stage has a single responsibility and produces observations for the following stage.
...
Component Extractors
      │
      ▼
Semantic Component Observations
      │
      ▼
Relationship Extractors
      │
      ▼
Semantic Relationship Observations
      │
      ▼
Semantic Validator
      │
...
Component extractors identify semantic entities directly from the observed geometry.
...
Each extractor is independent and has no knowledge of other semantic components.

### Relationship Extractors

Relationship extractors operate exclusively on semantic observations generated by the component stage.

They never analyze geometry directly.
...
This separation allows the semantic graph to grow by simply adding new relationship extractors without changing the component extraction pipeline.

### Design Principle

Semantic meaning is built incrementally.

The engine first discovers **what exists** (semantic components) and only afterwards derives **how these components are related** (semantic relationships).

This layered architecture keeps perception modular, extensible, and consistent with the project's perception-driven design philosophy.
