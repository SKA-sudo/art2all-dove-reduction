...
within a completed reference model and to transform them into validated
semantic knowledge.
...
The completed reference model is treated as the observable solution.
...
Each engineering algorithm is analysed with one question:

> **What truthful semantic statement can this algorithm produce today?**
...
Research is not the final objective.

The objective is always the Art2all project.
...
Architecture is not created before implementation.
...
Large architectural redesigns are avoided.

Instead, knowledge grows through small validated implementation steps.
...
The objective is therefore not to invent new structures.
...
The Reduction Lab deliberately avoids designing architecture before
sufficient evidence exists.

Validated knowledge therefore becomes architecture.
...
Before implementing a new extractor, always ask:

• What does the current implementation already reveal?

• What is its semantic responsibility?

• Can it produce exactly one truthful Semantic Observation?
...
The purpose of this distinction is to ensure that implementation,
...
Human observers do not recognise the Peace Dove by analysing individual
geometric elements.

Recognition emerges through a hierarchy of perceptual structures.

Large-scale visual organisation dominates local geometric detail.
...
Existing engineering algorithms can be transformed into independent
Semantic Observations.

Each observation represents exactly one truthful semantic fact.
...
Most engineering algorithms already contain semantic information.

Large architectural redesign is therefore unnecessary.

Migration produces a more stable semantic architecture while preserving
existing implementation knowledge.

### Evidence

Multiple extractors were introduced without replacing the underlying
...
Semantic information is not limited to static geometry.

Behaviour between objects can represent semantic knowledge.
...
Semantic understanding grows through many small validated observations.

Knowledge does not appear as one large architectural design.

It accumulates through continuous semantic migration.

### Evidence

Every migration step introduced one additional validated Semantic
Observation.

Together these observations now describe significantly more semantic
knowledge than any individual implementation.
...
Current research confirms that perceptual knowledge can be discovered
through incremental semantic migration.
...
The following hypotheses are active research topics of the Art2all
Reduction Lab.
...
Multiple reduction experiments indicate that silhouette, primary gesture
and body organisation dominate recognition.

The exact perceptual threshold remains unknown.
...
Relationship Forces successfully introduced behavioural organisation
inside the PaperField.

### Current Evidence

Behavioural interactions already influence local organisation without
...
Accumulated Semantic Observations may naturally evolve into higher-order
semantic knowledge structures.

These structures may emerge without explicit architectural planning.

### Motivation

During semantic migration independently developed Semantic Observations
began forming meaningful semantic relationships.
...
Research exists to improve the final artwork.

A semantic observation without visual contribution remains incomplete.

### Current Evidence

The semantic migration has not yet reached the Visual Mapping stage.
...
The Perception Engine should grow by accumulating validated Semantic
Observations rather than by introducing increasingly complex algorithms.

### Motivation

Current development demonstrates continuous semantic growth through
incremental migration.

### Current Evidence

Each migration step has successfully introduced one additional semantic
...
Active hypotheses guide future implementation.

They are intentionally separated from validated knowledge.
...
The engineering implementation remains unchanged whenever possible.
...
Several important findings emerged during semantic migration.

### Existing engineering code already contains semantic knowledge.
...
Several engineering components originally mixed multiple semantic ideas.

Migration separates these responsibilities into independent Semantic
Observations.
...
Replacing existing implementations risks losing accumulated engineering
experience.

Semantic migration preserves this knowledge while making its semantic
meaning explicit.
...
The LeftWing entity currently contains semantic knowledge describing the
internal organisation of the left wing.
...
│              │
▼              ▼
HAS_WING_   HAS_RELATIONSHIP_
FINGER_     FORCES
CURVES
```

The current semantic structure represents the present state of validated
knowledge.

It should not be interpreted as a fixed ontology.
...
This prevents architectural assumptions from becoming embedded in the
Perception Engine.
...
It emerged naturally from implementation.
...
The Perception Engine is not designed to reach a fixed final state.
...
Every validated Semantic Observation slightly increases the semantic
understanding of the reference model.

No single observation is expected to describe the complete Peace Dove.
...
As semantic knowledge grows, observations begin to reference related
semantic entities.
...
Semantic knowledge is expected to improve over time.
...
Every completed research cycle should increase the overall understanding
of perception.
...
The Reduction Lab is not intended to become a complete theory of human
perception.

Its objective is considerably more focused.

The objective is to discover the smallest amount of perceptual and
semantic knowledge required to construct the Art2all Peace Dove while
...
Research is not the destination.

Research is preparation.

The destination is the Art2all project.

Every validated discovery should eventually become part of a living,
interactive Peace Dove built from children's drawings.
...
The Reduction Lab does not attempt to replace artistic intuition.

It attempts to understand it.

The purpose of this research is not to reduce art to algorithms.
